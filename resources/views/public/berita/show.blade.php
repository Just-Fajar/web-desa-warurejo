                ...
                <div class="lg:col-span-8">
                    <article class="bg-white scroll-reveal">
                        ...
                        <div
                            ...
                            <span
                                class="text-gray-800">{{ $berita->admin->name ?? $berita->admin->username ?? 'Administrator' }}</span>
                            <span class="mx-2 text-gray-300">-</span>
                            <span class="text-[#e32] font-semibold">Berita</span>
                            <span class="mx-2 text-gray-300">|</span>
                            <span>{{ \Carbon\Carbon::parse($berita->published_at ?? $berita->created_at)->locale('id')->isoFormat('dddd, D MMM Y HH:mm') }}
                                WIB</span>
                        </div>
                        ...
                        <div class="mt-8 pt-6 border-t border-gray-100 flex items-center gap-4">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <a href="#"
                                    onclick="event.preventDefault(); navigator.clipboard.writeText(window.location.href); alert('Tautan berita ini tersalin ke clipboard! Silahkan buka aplikasi Instagram Anda.');"
                            ...
                            <div class="mt-8 pt-4">
                                ...
                                <div class="flex flex-wrap gap-2">
                                    @foreach(explode(',', $berita->tags) as $tag)
                                        <span
                                            class="px-4 py-2 bg-[#f1f1f1] text-[#333] text-[13px] sm:text-[14px] font-medium rounded-sm hover:bg-gray-200 transition cursor-pointer">
                                            {{ trim($tag) }}
                                        </span>
                                    @endforeach
                                </div>
                                    ...
                                    <div class="border-b border-gray-200 pb-3">
                                        <a href="{{ route('berita.show', $rekomendasi->slug) }}" class="flex group">
                                            @if($rekomendasi->gambar_utama_url)
                                                <div class="w-20 h-16 shrink-0 bg-gray-100 rounded-md overflow-hidden mr-3">
                                                    <img src="{{ $rekomendasi->gambar_utama_url }}"
                                                        class="w-full h-full object-cover group-hover:scale-105 transition duration-300">
                                                </div>
                                            @endif
                                            <div class="flex-1">
                        ...
                        <div class="mt-10 pt-8 border-t border-gray-200 mb-10">
                            <div class="flex items-center justify-between mb-6">
                                ...
                                <a href="{{ route('berita.index') }}"
                                    class="text-sm font-semibold text-[#e32] hover:text-[#003366]">Selengkapnya &rarr;</a>
                            </div>
                                    ...
                                    <a href="{{ route('berita.show', $lainnya->slug) }}" class="group block">
                                        <div class="w-full h-36 bg-gray-200 rounded-lg overflow-hidden mb-3">
                                            @if($lainnya->gambar_utama_url)
                                                <img src="{{ $lainnya->gambar_utama_url }}"
                                                    class="w-full h-full object-cover group-hover:scale-105 transition duration-300">
                                            @endif
                                        </div>
                                        <p class="text-[13px] text-[#e32] mb-1">Berita Desa</p>
                                        <h3
                                            class="text-[15px] font-bold text-gray-900 group-hover:text-[#003366] leading-tight line-clamp-3">
                                            {{ $lainnya->judul }}
                                        </h3>
                                    </a>
                                @endforeach
                        ...
                        @if(isset($relatedBerita) && $relatedBerita->count() > 0)
                            <div class="mb-8">
                                ...
                                <div class="space-y-4">
                                    @foreach($relatedBerita->take(5) as $index => $populer)
                                        <a href="{{ route('berita.show', $populer->slug) }}" class="flex group">
                                            <div
                                                class="text-3xl font-extrabold text-gray-300 mr-4 mt-1 italic group-hover:text-[#e32] transition">
                                                #{{ $index + 1 }}
                                            </div>
                                            <div>
                                                <h3
                                ...
                                <div class="mt-6 text-center">
                                    <a href="{{ route('berita.index') }}"
                                        class="inline-block bg-gray-100 hover:bg-gray-200 text-[#e32] font-semibold text-sm py-2 px-6 rounded-md transition">Lihat
                                        Selengkapnya &rarr;</a>
                                </div>
                            </div>
                        @endif
