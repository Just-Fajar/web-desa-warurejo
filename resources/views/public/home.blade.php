        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 items-stretch card-grid content-grid mobile-slider">
            @forelse($latest_berita as $berita)
                ...
                <div class="bg-white rounded-2xl p-3 border border-gray-100 shadow-sm hover:shadow-xl hover:border-primary-100 transition-all duration-300 group flex flex-col scroll-reveal-stagger ring-1 ring-black/5 hover:ring-primary-500/20">
                    ...
                    {{-- Image Wrapper --}}
                    <a href="{{ route('berita.show', $berita->slug) }}" class="relative block h-56 rounded-xl overflow-hidden mb-4">
                        ...
                        {{-- Date Badge --}}
                        <div class="absolute top-3 left-3 bg-white/95 backdrop-blur-sm px-2.5 py-1.5 rounded-lg text-xs font-semibold text-gray-700 flex items-center gap-1.5 shadow-sm">
                    ...
                    <div class="px-2 pb-2 flex flex-col grow">
                        ...
                        {{-- Action Area --}}
                        <a href="{{ route('berita.show', $berita->slug) }}" class="mt-auto pt-4 border-t border-gray-50 flex items-center justify-between group/action">
            ...
            @forelse($potensi->take(3) as $item)
                ...
                <div class="bg-white rounded-2xl p-3 border border-gray-100 shadow-sm hover:shadow-xl hover:border-primary-100 transition-all duration-300 group flex flex-col scroll-reveal-stagger ring-1 ring-black/5 hover:ring-primary-500/20">
                    ...
                    <div class="px-2 pb-2 flex flex-col grow">
                        ...
                        {{-- Action Area --}}
                        <a href="{{ route('potensi.show', $item->slug) }}" class="mt-auto pt-4 border-t border-gray-50 flex items-center justify-between group/action">
                            <span class="text-primary-600 font-semibold text-sm group-hover/action:text-primary-700 transition-colors">
                                Selengkapnya
                            </span>
                            <span class="w-8 h-8 rounded-full bg-primary-50 text-primary-600 flex items-center justify-center group-hover/action:bg-primary-600 group-hover/action:text-white transition-all duration-300 transform group-hover/action:translate-x-1">
...
<section class="py-16 md:py-24 bg-white content-section relative overflow-hidden">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 main-content max-w-7xl">
        <div class="text-center mb-10 md:mb-16 section-header scroll-reveal">
            ...
            <p class="text-gray-500 text-lg md:text-xl">Potret keindahan, kegiatan warga, dan pembangunan di {{ $profil->nama_desa }}.</p>
        </div>
        ...
        @php
            ...
            foreach($kategoriList as $index => $kat) {
                ...
                $matched = $galeri->first(function($item) use ($kat, $usedIds) {
                    if (in_array($item->id ?? $item->id_galeri ?? -1, $usedIds)) return false;
                    if ($kat['nama'] === 'Lainnya') return false;
        ...
        <div class="flex flex-col md:flex-row w-full h-[600px] gap-2 md:gap-4 scroll-reveal" id="gallery-accordion">
            @foreach($galeriDisplay as $index => $item)
                <div class="gallery-item relative overflow-hidden rounded-2xl md:rounded-[2rem] transition-all duration-700 ease-[cubic-bezier(0.4,0,0.2,1)] cursor-pointer {{ $index == 0 ? 'flex-[4] active' : 'flex-1' }}"
                     onmouseenter="activateGalleryItem(this)"
                     onclick="openImageModal('{{ $item['image'] }}')">
                     
                     <img src="{{ $item['image'] }}" alt="{{ $item['kategori'] }}" class="absolute inset-0 w-full h-full object-cover object-center transition-transform duration-1000 gallery-img group-hover:scale-105">
                     
                     {{-- Gradient Overlay --}}
                     <div class="absolute inset-0 bg-gradient-to-t from-gray-900/90 via-gray-900/40 to-gray-900/10 transition-opacity duration-700"></div>

                     {{-- Vertical Content (Muncul saat tidak di-hover) --}}
                     <div class="content-vertical absolute inset-0 flex items-center justify-center md:items-end md:justify-center md:pb-12 transition-opacity duration-300 opacity-100">
                         <h3 class="text-white font-extrabold tracking-[0.3em] text-sm md:text-xl uppercase whitespace-nowrap glow-text writing-vertical">
                             {{ strtoupper($item['kategori']) }}
                         </h3>
                     </div>

                     {{-- Horizontal Content (Muncul saat di-hover/aktif) --}}
                     <div class="content-horizontal absolute bottom-0 left-0 p-6 md:p-10 w-full bg-gradient-to-t from-black via-black/80 to-transparent flex flex-col justify-end opacity-0 transform translate-y-8 transition-all duration-500 delay-100 pointer-events-none">
                         <div class="flex items-center gap-4 mb-3 md:mb-4">
                             <div class="w-10 h-1 bg-primary-500 rounded-full"></div>
                             <h3 class="text-2xl md:text-3xl lg:text-4xl font-bold text-white whitespace-nowrap truncate">{{ $item['kategori'] }}</h3>
                         </div>
                         <p class="text-gray-200 text-sm md:text-base line-clamp-2 md:line-clamp-3 md:pl-14 opacity-90 max-w-2xl font-medium">
                             {{ $item['desc'] }}
                         </p>
                     </div>
                </div>
